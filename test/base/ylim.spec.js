...
], function (sinon, $J, CommandParser) {

$J.describe("cali.module.base.ylim(h, [ymin, ymax])", function() {
	...
	describe("integrates with the command parser", function() {
		...
		describe("ylim(h, [xmin, xmax])", function() {
			...
			it("should call editPlot with the id, and new plot data", function(){
				...
				for(var i = 0; i < ylims.length; i++){
					parser.evaluate(plot[i], function(err, ans){
						var id = ans[0];
						parser.evaluate(ylims[i], function(err, ans){
